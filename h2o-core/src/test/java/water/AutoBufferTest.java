...
public class AutoBufferTest extends TestUtil {
  ...
  public void testOutputStreamBigDataBigChunks() {
    Assume.assumeTrue("testOutputStreamBigDataBigChunks: JVM has enough memory", Runtime.getRuntime().maxMemory() > 5e9);
    ...
    ab.putA1(data);
    // try to stream 2TB of data through AutoBuffer
    try {
      ...
      // this is a known issue - AutoBuffer cannot continuously take large chunks of data (close to 2GB)
      // we don't care - that is not a real use case
      assertTrue(e.getMessage().startsWith("Cannot allocate more than 2GB array"));
    }
  }
